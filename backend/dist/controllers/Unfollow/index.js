...
const Unfollow = async (req, res) => {
    try {
        ...
        const user = await User_1.default.findOneAndUpdate({ username: req?.body?.username }, { $pull: { followers: { username: decoded.user.username } } }, { new: true });
        //! Here update me followings list
        const me = await User_1.default.findOneAndUpdate({ username: decoded.user.username }, { $pull: { followings: { username: req?.body.username } } }, { new: true });
        if (!user) {
