...
const DeleteComment = async (req, res) => {
    try {
        ...
        await Post_1.default.updateMany({ _id: req.body.from }, { $pull: { comment: req.body } });
        const post = await Post_1.default.findOne({ _id: req.body.from });
        //! Add new comment for posts user schema
        await User_1.default.updateMany({ username: post?.owner?.name }, { $pull: { "posts.$[].comment": req.body } });
